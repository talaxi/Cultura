  ...
  runRace(race: Race, expediteRace?: boolean): RaceResult {
    ...
    race.raceLegs.forEach(item => {
      ...
      if (framesPassed < this.timeToComplete * this.frameModifier) {
        ...
        if (!(this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix &&
          this.globalService.globalVar.eventRaceData.animalAlreadyPrepped))
          this.PrepareRacingAnimal(raceResult, framesPassed, lastAnimalDisplayName, racingAnimal, completedAnimals, item, lastLeg, undefined, undefined, previousRacerVelocity);
      ...
      if (deepBreathingRelayEffect !== undefined && deepBreathingRelayEffect.additionalValue !== undefined) {
        ...
        if (allDeepBreathingRelayEffects !== undefined) {
          allDeepBreathingRelayEffects.forEach(effect => {
            ...
            //console.log("DBSG Increase: " + deepBreathingStaminaGain);
      ...
      //console.log("Pre Leg Relay Effect Count Check: " + racingAnimal.raceVariables.relayEffects.length);
      ...
      for (var i = framesPassed; i <= this.timeToComplete * this.frameModifier; i++) {
        ...
        var modifiedAdaptabilityMs = this.getModifiedAdaptability(racingAnimal, permanentAdaptabilityIncreaseMultiplierObj, statLossFromExhaustion, nineTailsBuffs, item, currentDistanceInLeg, nextAnimal, nextLeg, raceResult, framesPassed, animalDisplayName, nextAnimalDisplayName);
        ...
        if (doesRacerBurst || herdMentalityBurst) {
          ...
          if (racingAnimal.type === AnimalTypeEnum.Octopus && racingAnimal.ability.name === "Big Brain") {
            ...
            if (racingAnimal.raceVariables.burstCount === burstCap) {
              ...
              this.AddRelayEffect(new Animal(), length, new AnimalStats(1, 1, 1, powerMultiplier, 1, 1), true, RelayEffectEnum.bigBrain, true);
        ...
        if (racingAnimal.type === AnimalTypeEnum.Goat && racingAnimal.ability.name === "Deep Breathing") {
          ...
          if (racingAnimal.currentStats.stamina > animalMaxStamina) {
            ...
            this.AddRelayEffect(new Animal(), distance, new AnimalStats(0, 0, 0, 0, 0, 0), false, RelayEffectEnum.deepBreathing, true, staminaGain);

            //console.log("New DBSG: " + staminaGain);
      ...
      race.raceUI.distanceCoveredByFrame.push(Math.round(distanceCovered * 1e5) / 1e5);
      race.raceUI.velocityByFrame.push(Math.round(distanceCoveredPerFrame * 1e5) / 1e5);
      if (!(this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix))
        race.raceUI.timeToCompleteByFrame.push(this.timeToComplete);
      race.raceUI.maxSpeedByFrame.push(Math.round(modifiedMaxSpeed * 1e5) / 1e5);
      race.raceUI.staminaPercentByFrame.push((Math.round((racingAnimal.currentStats.stamina / animalMaxStamina) * 1e5) / 1e5));
      ...
      if (distanceToGo <= 0) {
        //console.log("Finish Relay Length Check: " + racingAnimal.raceVariables.relayEffects.length);
        raceResult.addRaceUpdate(framesPassed, animalDisplayName + " finishes their leg of the race.");
        item.legComplete = true;
        legCounter += 1;

        this.preRacerResetRelayEffectCheck(racingAnimal, nextAnimal, raceResult, framesPassed, lastAnimalDisplayName, item, item, statLossFromExhaustion, modifiedVelocity);
        ...
        if (!(this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix &&
          ...
          this.endOfLegRelayEffectCheck(racingAnimal, new Animal(), raceResult, framesPassed, lastAnimalDisplayName, item, item, statLossFromExhaustion, modifiedVelocity);
        }
    ...
    if (this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix) {
      ...
      this.globalService.globalVar.eventRaceData.previousRaceSegmentData = this.carryOverEventRaceSegmentData(racingAnimal, velocity, permanentMaxSpeedIncreaseMultiplier, permanentAdaptabilityIncreaseMultiplierObj.permanentAdaptabilityIncreaseMultiplier,
        ...
        coldBloodedIncreaseMultiplierObj.coldBloodedIncreaseMultiplier, fleetingSpeedIncreaseMultiplier, nineTailsBuffs,
        deepBreathingStaminaGain);
    }
  });
...
getBurstDistance(animal: Animal, terrainModifier: number, statLossFromExhaustion: number) {
  ...
  return animal.currentStats.calculateBurstDistance(breedLevelStatModifierValue, undefined, modifiedPowerMs) * runningShoesModifier;
}
...
doesRacerBurst(animal: Animal, currentPath: RacePath, timeToComplete: number, raceLength: number, currentDistanceInRace: number, modifiedAdaptabilityMs: number, modifiedFocusMs: number): boolean {
  ...
  if (this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix) {
    var eventRaceModifier = 30 / this.globalService.globalVar.eventRaceData.segmentTime;
    modifiedBurstChance = modifiedBurstChance / eventRaceModifier;
  }
...
PrepareRacingAnimal(raceResult: RaceResult, framesPassed: number, lastAnimalDisplayName: string, animal: Animal, completedAnimals ?: Animal[], currentLeg ?: RaceLeg, previousLeg ?: RaceLeg, statLossFromExhaustion ?: number, raceFinished ?: boolean, modifiedVelocity ?: number) {
  if (this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix)
  ...
  if (raceFinished !== true) {
    if (completedAnimals !== undefined && completedAnimals !== null && completedAnimals.length > 0) {
      ...
      this.endOfLegRelayEffectCheck(lastCompletedAnimal, animal, raceResult, framesPassed, lastAnimalDisplayName, currentLeg, previousLeg, statLossFromExhaustion, modifiedVelocity);
    }
...
preRacerResetRelayEffectCheck(animalGivingRelayEffect: Animal, animalReceivingRelayEffect: Animal | null, raceResult: RaceResult, framesPassed: number, lastAnimalDisplayName: string, currentLeg ?: RaceLeg, previousLeg ?: RaceLeg, statLossFromExhaustion ?: number, modifiedVelocity ?: number, animalMaxStamina ?: number) {
  var relayAbilityXpIncrease = 4;
  var relayAbilityXpIncreasePair = this.globalService.globalVar.modifiers.find(item => item.text === "relayAbilityXpGainModifier");
  if (relayAbilityXpIncreasePair !== undefined)
    relayAbilityXpIncrease = relayAbilityXpIncreasePair.value;
  ...
  if (previousLeg !== null && previousLeg !== undefined) {
    if (animalGivingRelayEffect.type === AnimalTypeEnum.Caribou && animalGivingRelayEffect.ability.name === "Herd Mentality" && !animalGivingRelayEffect.raceVariables.ranOutOfStamina) {
      ...
      this.AddRelayEffect(animalReceivingRelayEffect, distance, new AnimalStats(0, 0, 0, 0, 0, 0), false, RelayEffectEnum.herdMentality, false, herdMentalityStaminaGain);
    ...
    if (animalGivingRelayEffect.type === AnimalTypeEnum.Caribou && animalGivingRelayEffect.ability.name === "Special Delivery" && !animalGivingRelayEffect.raceVariables.ranOutOfStamina) {
      ...
      var distance = this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix ?
              this.globalService.globalVar.eventRaceData.totalDistance : this.selectedRace.length;
      var specialDeliveryMaxSpeedBonus = ((animalGivingRelayEffect.currentStats.stamina / animalMaxStamina)) * (this.lookupService.GetAbilityEffectiveAmount(animalGivingRelayEffect, previousLeg.terrain.powerModifier, statLossFromExhaustion) / 100);
      this.AddRelayEffect(animalGivingRelayEffect, distance, new AnimalStats(0, 0, 0, 0, 0, 0), false, RelayEffectEnum.specialDelivery, true, specialDeliveryMaxSpeedBonus);
      raceResult.addRaceUpdate(framesPassed, lastAnimalDisplayName + " uses " + animalGivingRelayEffect.ability.name + ".");
      this.globalService.increaseAbilityXp(animalGivingRelayEffect, relayAbilityXpIncrease);
    }
  }
...
endOfLegRelayEffectCheck(animalGivingRelayEffect: Animal, animalReceivingRelayEffect: Animal, raceResult: RaceResult, framesPassed: number, lastAnimalDisplayName: string, currentLeg ?: RaceLeg, previousLeg ?: RaceLeg, statLossFromExhaustion ?: number, modifiedVelocity ?: number) {
  var relayAbilityXpIncrease = 4;
  var relayAbilityXpIncreasePair = this.globalService.globalVar.modifiers.find(item => item.text === "relayAbilityXpGainModifier");
  if (relayAbilityXpIncreasePair !== undefined)
    relayAbilityXpIncrease = relayAbilityXpIncreasePair.value;
  ...
  if (animalGivingRelayEffect.ability.name === "Inspiration" && animalGivingRelayEffect.type === AnimalTypeEnum.Horse) {
    if (previousLeg !== undefined && previousLeg !== null) {
      ...
      this.AddRelayEffect(animalReceivingRelayEffect, length, new AnimalStats(additiveAmount, 0, 0, 0, 0, 0), false, RelayEffectEnum.inspiration, false);
      raceResult.addRaceUpdate(framesPassed, lastAnimalDisplayName + " uses " + animalGivingRelayEffect.ability.name + ".");
      this.globalService.increaseAbilityXp(animalGivingRelayEffect, relayAbilityXpIncrease);
    }
  }
  ...
  if (animalGivingRelayEffect.ability.name === "Flowing Current" && animalGivingRelayEffect.type === AnimalTypeEnum.Dolphin) {
    if (previousLeg !== undefined && previousLeg !== null) {
      ...
      this.AddRelayEffect(animalReceivingRelayEffect, length, new AnimalStats(0, additiveAmount, 0, 0, 0, 0), false, RelayEffectEnum.flowingCurrent, false);
      raceResult.addRaceUpdate(framesPassed, lastAnimalDisplayName + " uses " + animalGivingRelayEffect.ability.name + ".");
      this.globalService.increaseAbilityXp(animalGivingRelayEffect, relayAbilityXpIncrease);
    }
  }
  ...
  if (animalGivingRelayEffect.ability.name === "Navigator" && animalGivingRelayEffect.type === AnimalTypeEnum.Dolphin) {
    if (previousLeg !== undefined && previousLeg !== null) {
      ...
      this.AddRelayEffect(animalReceivingRelayEffect, length, new AnimalStats(0, 0, 0, 0, 0, additiveAmount), false, RelayEffectEnum.navigator, false);
      raceResult.addRaceUpdate(framesPassed, lastAnimalDisplayName + " uses " + animalGivingRelayEffect?.ability.name + ".");
      this.globalService.increaseAbilityXp(animalGivingRelayEffect, relayAbilityXpIncrease);
    }
  }
  ...
  if (animalGivingRelayEffect.type === AnimalTypeEnum.Gecko && animalGivingRelayEffect.ability.name === "Camouflage") {
    if (previousLeg !== undefined && previousLeg !== null) {
      ...
      var length = this.lookupService.GetAbilityEffectiveAmount(animalGivingRelayEffect, previousLeg.terrain.powerModifier, statLossFromExhaustion);
      this.AddRelayEffect(animalReceivingRelayEffect, length, new AnimalStats(0, 0, 0, 0, 0, 0), false, RelayEffectEnum.camouflage, false, additionalEffect);
      raceResult.addRaceUpdate(framesPassed, lastAnimalDisplayName + " uses " + animalGivingRelayEffect.ability.name + ".");
    }
  }

  if (currentLeg !== undefined)
    this.batonEquipmentCheck(animalGivingRelayEffect, animalReceivingRelayEffect, currentLeg.distance);
}
...
batonEquipmentCheck(previousAnimal: Animal, nextAnimal: Animal, length: number) {
  ...
  if (previousAnimal.getEquippedItemName() === this.globalService.getEquipmentName(EquipmentEnum.redBaton)) {
    ...
    this.AddRelayEffect(nextAnimal, length, new AnimalStats(defaultRedBatonModifier, 1, 1, 1, 1, 1), true, RelayEffectEnum.redBaton, false);
  }
  if (previousAnimal.getEquippedItemName() === this.globalService.getEquipmentName(EquipmentEnum.blueBaton)) {
    ...
    this.AddRelayEffect(nextAnimal, length, new AnimalStats(1, 1, 1, 1, defaultBlueBatonModifier, 1), true, RelayEffectEnum.blueBaton, false);
  }
  if (previousAnimal.getEquippedItemName() === this.globalService.getEquipmentName(EquipmentEnum.violetBaton)) {
    ...
    this.AddRelayEffect(nextAnimal, length, new AnimalStats(1, 1, 1, defaultVioletBatonModifier, 1, 1), true, RelayEffectEnum.violetBaton, false);
  }
  if (previousAnimal.getEquippedItemName() === this.globalService.getEquipmentName(EquipmentEnum.greenBaton)) {
    ...
    this.AddRelayEffect(nextAnimal, length, new AnimalStats(1, 1, 1, 1, 1, defaultGreenBatonModifier), true, RelayEffectEnum.greenBaton, false);
  }
  if (previousAnimal.getEquippedItemName() === this.globalService.getEquipmentName(EquipmentEnum.orangeBaton)) {
    ...
    this.AddRelayEffect(nextAnimal, length, new AnimalStats(1, defaultOrangeBatonModifier, 1, 1, 1, 1), true, RelayEffectEnum.orangeBaton, false);
  }
  if (previousAnimal.getEquippedItemName() === this.globalService.getEquipmentName(EquipmentEnum.yellowBaton)) {
    ...
    this.AddRelayEffect(nextAnimal, length, new AnimalStats(1, 1, defaultYellowBatonModifier, 1, 1, 1), true, RelayEffectEnum.yellowBaton, false);
  }
}
...
AddRelayEffect(racingAnimal: Animal, distance: number, statMultiplers: AnimalStats, isMultiplicative: boolean, type: RelayEffectEnum, affectsAllRacers: boolean, additionalValue ?: number) {
  ...
  if (this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix) {
    ...
    //needs to handle previous racer as well
    if (affectsAllRacers) {
...
useAbility(racingAnimal: Animal, race: Race, currentLeg: RaceLeg, statLossFromExhaustion: number, animalMaxStamina: number, currentDistanceInLeg: number, obj: { coldBloodedIncreaseMultiplier: number }) {
  ...
  if (racingAnimal.type === AnimalTypeEnum.Salamander) {
    if (racingAnimal.ability.name === "Cold Blooded") {
      ...
      var coldBloodedStaminaModifier = percentInLeg * .6;//brings 50% down to 30%

      racingAnimal.currentStats.stamina += animalMaxStamina * coldBloodedStaminaModifier;
      if (racingAnimal.currentStats.stamina > animalMaxStamina)
        racingAnimal.currentStats.stamina = animalMaxStamina;
      obj.coldBloodedIncreaseMultiplier = 1 + coldBloodedStaminaModifier;
    }
...
handleRacePositionByFrame(race: Race, averageDistancePerSecond: number, framesPassed: number, distanceCovered: number) {
  ...
  if (race.localRaceType === LocalRaceTypeEnum.Track) {
    ...
    if (this.selectedRace.raceType === RaceTypeEnum.event && this.selectedRace.eventRaceType === EventRaceTypeEnum.grandPrix) {
      var timePassed = (this.globalService.globalVar.eventRaceData.grandPrixTimeLength + this.globalService.globalVar.eventRaceData.bonusTime) - this.globalService.getRemainingEventRaceTime();
      var averageDistancePassed = averageDistancePerSecond * timePassed;
      averageDistance += averageDistancePassed;

      //console.log("Total Time: " + this.globalService.globalVar.eventRaceData.grandPrixTimeLength + " Remaining Time: " + this.globalService.getRemainingEventRaceTime() + " = " + timePassed);
      //console.log("ADPS: " + averageDistancePerSecond + " AVG Distance: " + averageDistance + " Past Distance: " + averageDistancePassed);
    }
...
carryOverEventRaceSegmentData(racingAnimal: Animal, velocity: number, permanentMaxSpeedIncrease: number, permanentAdaptabilityIncrease: number,
  ...
  fleetingSpeedIncrease: number, nineTailsBuffs: [string, number][], deepBreathingStaminaGain: number) {
